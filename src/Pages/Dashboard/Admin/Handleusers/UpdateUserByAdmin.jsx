...
const UpdateUserByAdmin = () => {
    ...
    const { refetch, data: profile = {}, isLoading: profileLoading, error } = useQuery({
        queryKey: ['userProfileForAdmin', email],
        queryFn: async () => {
            const res = await axiosSecure.get(`/get-user-profile/${email}`);
    ...
    const uploadToServer = data => {
        ...
        axiosSecure.patch(`/update-user-profile-by-admin/${email}`, data)
            .then(result => {
